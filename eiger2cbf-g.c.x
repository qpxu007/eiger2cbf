...
const int frame_renumber(int frame, float osc, float osc0, float osc_width) {
        int modified_frame;
        modified_frame = (int)round((osc - osc0) / osc_width + 1);
...
int main(int argc, char **argv)
{
  ...
  fprintf(stderr, "EIGER HDF5 to CBF converter (version 160530-qxumod)\n");
  ...
  {
    printf("Usage:\n");
    printf("  %s filename.h5           -- extract all frames with the same filename pattern\n", argv[0]);
...
//    int opt;
//    while ((opt = getopt(argc, argv, "s:e:p:")) != -1) {
//        switch (opt) {
//            case 's':
//                from = (int)optarg;
//                break;
...
// default output file prefix
  //char *master_file = extractFilename(argv[1]);
  char *location = strstr(master_file, "master.");
  int prefix_len = location - master_file;
  //char *prefix[prefix_len + 1];
  strncpy(prefix, master_file, prefix_len);
  prefix[prefix_len] = '\0';
  ...
  for (frame = from; frame <= to; frame++)
  {
    ...
    if (argc >= 2)
    {
      ...
      else
      {
        ...
        // recalculate frame number based on actual omega values
        int modified_frame;
        if (osc_width >= 1e-6)
        {
          modified_frame = frame_renumber(frame, osc_start, angles[0], osc_width);
        }
        else
        {
        ...
        if (argc > 2)
        {
          snprintf(filename, 4096, "%s%06d.cbf", argv[3], modified_frame);
        }
        else if (argc == 2)
        {
